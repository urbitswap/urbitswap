// copied from `tloncorp/landscape-apps`
// https://github.com/tloncorp/landscape-apps/blob/f9304a403a00cadfe0dca9ba719cb56298cf611b/ui/src/state/vitals.ts
...
export function useConnectivityCheck(
  ...
  const query = useQuery(
    ...
    async (): Promise<ConnectionUpdate> => {
      const resp = await urbitAPI.scry<ConnectionUpdate>({
        app: 'vitals',
        path: `/ship/${ship}`,
      });
      const now = Date.now();
